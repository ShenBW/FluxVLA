...
class TestDreamZeroHeadTiny(unittest.TestCase):
    ...
    def test_forward_produces_loss(self):
        ...
        out['loss'].backward()
        grads = [
            p.grad for p in self.head.model.parameters() if p.grad is not None
        ]
    ...
    def test_predict_action_cache_mode_reuses_history(self):
        ...
        cache = self.head.inference_kv_cache
        self.assertIsNotNone(cache)
        self.assertEqual(self.head.current_start_frame, 3)

        torch.manual_seed(123)
        with torch.no_grad():
            continued = self.head.predict_action(
                num_inference_steps=2,
                **inputs,
            )
        self.assertIs(self.head.inference_kv_cache, cache)
        self.assertEqual(self.head.current_start_frame, 5)
    ...
    def test_predict_action_cache_mode_single_frame_forces_reset(self):
        ...
        # The VLM produces one observed latent but conditioning for the
        # entire generated block (including the future masked frames).
